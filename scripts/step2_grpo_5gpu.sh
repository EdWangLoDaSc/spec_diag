#!/usr/bin/env bash
# Run spec_diag GRPO training on all 5 GPUs (0,1,2,3,4).
#
...
# Crosses NUMA, so the NCCL_SHM_DISABLE workaround from the 4-GPU script
# is required here too.
#
# NOTE: this script uses ALL GPUs for training, so the generator's vLLM
# endpoint (OPENAI_BASE_URL) must run on a DIFFERENT node / job — it cannot
# share a GPU with a training worker. Point OPENAI_BASE_URL at that server.
...
# 5-GPU tuned defaults (batch must be divisible by 5)
...
ACTOR_MINI_BATCH_SIZE="${ACTOR_MINI_BATCH_SIZE:-40}"
...
# NCCL fixes for cross-NUMA (GPU0/1/2 on NUMA0, GPU3/4 on NUMA1)
...
if [ "$N_VIS" -ne 5 ]; then
  echo "ERROR: GRPO script needs exactly 5 visible GPUs; got CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES" >&2
  exit 2
fi

# CRITICAL: Ray hangs at init_workers() if torch cannot see exactly 5 GPUs.
_N_CUDA="$(python -c 'import torch; print(torch.cuda.device_count())' 2>/dev/null || echo 0)"
if [ "$_N_CUDA" -ne 5 ]; then
  echo "ERROR: torch.cuda.device_count()=${_N_CUDA}, but we requested 5 GPUs." >&2
  echo "       CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES" >&2
  echo "       Nothing else can be using these GPUs (including local vLLM)." >&2
  exit 2
fi
unset _N_CUDA
...
if ! curl -sf "${OPENAI_BASE_URL}/models" >/dev/null 2>&1; then
  ...
  echo "5-GPU mode needs vLLM on a DIFFERENT node; set OPENAI_BASE_URL." >&2
  exit 3
fi
...
echo "==== GRPO split launcher (5 GPU - NUMA aware) ===="
echo "host         : $(hostname)"
...
python -m spec_diag.train \
  ...
  trainer.experiment_name="split_5grpo_${TS}" \
