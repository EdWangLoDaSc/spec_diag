#!/usr/bin/env bash
# Run spec_diag GRPO training on exactly 2 GPUs.
...
if [ "$N_VIS" -ne 2 ]; then
  echo "ERROR: GRPO script needs exactly 2 visible GPUs; got CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES" >&2
  exit 2
fi
...
VERL_HYDRA_CONFIG_DIR="${VERL_HYDRA_CONFIG_DIR:-$SPEC_DIAG_ROOT/verl/verl/trainer/config}"
